...
.EXAMPLE
    ./Scripts/Test-BranchPathPolicy.ps1 -BranchName 'copilot/fix-workflow' -ChangedPath @('.github/workflows/publish.yml')
    # Fails: bare AI-root branches block ALL paths and must be renamed to -code/ or -infra/ before merge
.NOTES
...
# Bare AI-root branches are a "rename required" gate: any change at all blocks the merge.
# Handled before per-path checks so new top-level directories can never slip through.
$blocksAllPaths = ($matchingBranchPolicy.PSObject.Properties.Name -contains 'BlockAllPaths') -and
    $matchingBranchPolicy.BlockAllPaths

if ($blocksAllPaths -and $ChangedPath.Count -gt 0) {
    Write-Error -ErrorAction Continue @"
Branch path policy failed.

Branch '$BranchName' is treated as a $branchType.
This branch type may not merge into main with any changes.

Blocked changed paths:
$($ChangedPath -join [Environment]::NewLine)

Rename the branch to a '-code/' prefix (for source changes) or a '-infra/' prefix
(for workflow/infrastructure changes) before merging.
"@
    exit $script:PolicyViolationExitCode
}

$blockedPrefixes = @(
    if ($matchingBranchPolicy.PSObject.Properties.Name -contains 'BlockedPath') {
        $matchingBranchPolicy.BlockedPath
    }
)
$allowedPaths = @(
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBntC2iGfQ5HXWL
# kxIOpr7jAAPiZg95L3f53DpybPMYX6CCG1gwggN5MIIC/qADAgECAhAcz51nzeIZ
...
# BCCUuGbLQoWd4J/Nujg5oCYgSce3+PrBe/k7z5ovuRWZljALBgcqhkjOPQIBBQAE
# aDBmAjEA2KApEPBh8Elo0qgRAi72gurV2gl9A3oTydR0LVxegH07mhdduRh9RwGu
# UPiU2FHPAjEA9nczmusEg/bLwDRmX9LqSWlntId0hCxDsGCMzyZczJ+uiWuLfGfw
# Xk4CnVQy7P0/oYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkG
...
# BgkqhkiG9w0BCQUxDxcNMjYwODMwMjMyODM0WjA/BgkqhkiG9w0BCQQxMgQwFVHg
# 2XQW3mdnUjDg5kAwbbqLUIDcsGH7AJMmGr+gDu5KoVk9XpeCk2sWBELZXNyiMA0G
# CSqGSIb3DQEBAQUABIICAE4pUswZXZl5U7KhRkW5I6LKlslrDAoti37SV5MloZ84
# 6Utry6Q5Zmy1S5npCK/5n42TziPpSPKw9x+3/9kXFAozwBsFg1cP9uYxPGi3H9DK
# Z+81AyRLfKmBuUWY1FxQne0FjNJuk0vJmhN4Z2wOr1cRZiwEbGW+KCHqNJvhVQ5+
# jkCue6IHP44tC0qWAol7n1QwFuFwDB5SkHvP9jQwNhV+ShnuI20672jpFQm6V02H
# DF1sSfLy+yR4GYvzFR3f1FxFXXMqgYcrg4w5knJLYt2NJMMm3r7SayrUPCyd8a2i
# FpT8C+pfhx4pz0XQAasnCXox/ulQUFSbvYqST1OT315ddnAZohpiOR1v1C7/NoQn
# IJJAceEUSWaxgyfqYP0MP1g4dis62h+AOc8ztMRJlfeZZc+JzJmy26YNBkygQ+He
# BaT1AsRTbQrHq4hSDvbzjv7uca1b1z034WvV421xI9pwBSMu+9CgGeDnw0FL0KeR
# q1tNOWCX++V+sLyZG7l9SN4OJx3NxWbeIolX+Cks3CZu7Y/GGSsWLDdMnUUS2hPV
# ZKHgju1APE7vqRFehFBZ6YtI3PZiwjuJcUw3mzXiDrAeuDr4exrrHuL3t4Ir+77c
# Pot1xsoEYTPxteXdB3m4+WPXr8y7qFNd5wTOnbX0TdkpTsOqgwdPw2diUYrAobbh
