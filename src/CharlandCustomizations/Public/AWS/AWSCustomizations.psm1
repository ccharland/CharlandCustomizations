...
function Import-CHARPfxCertificateToACM {
  <#
.SYNOPSIS
  Converts an exported PFX certificate and imports it into AWS ACM.
...
#>
  ...
  process {
    try {
      ...
      $importParams = @{
        Certificate = [System.Text.Encoding]::ASCII.GetBytes($certificatePem)
        PrivateKey  = [System.Text.Encoding]::ASCII.GetBytes($privateKeyPem)
        ErrorAction = 'Stop'
      }

      if ($chainPemEntries.Count -gt 0) {
        $importParams['CertificateChain'] = [System.Text.Encoding]::ASCII.GetBytes(($chainPemEntries -join "`n"))
      }
      if ($CertificateArn) {
      ...
      if ($PSCmdlet.ShouldProcess($target, "Import certificate from '$resolvedPfxPath'")) {
