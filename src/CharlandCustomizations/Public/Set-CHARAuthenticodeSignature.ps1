...
#>
    ...
    begin {
           if (-not (Get-Command Test-CHARIsWindows -ErrorAction SilentlyContinue)) {
             . (Join-Path $PSScriptRoot '../Private/Test-CHARIsWindows.ps1')
         }

        if (-not (Test-CHARIsWindows)) {
        ...
        if ($TimeStampServer -eq '') {
            ...
            elseif ($MyCert.Issuer.StartsWith('CN=SSL')) {
                Write-Verbose 'SSL.com cert found'
                # use sectigo timestamp server - ssl.com timestamp service doesn't respond properly
                $TimeStampServer = 'http://timestamp.sectigo.com'
            }
            else {
                ...
                Write-Verbose "SSL.Com? $($MyCert.Issuer.StartsWith('CN=SSL'))"
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAwb/sutJR8THXk
# f+st5BYiMpYJzTNWOrmNQUKMouM6HKCCG1gwggN5MIIC/qADAgECAhAcz51nzeIZ
...
# BCADe0KWfTjbE6FUmT6FPbBRxoEtMv5ecvt+4hWoQV+IzTALBgcqhkjOPQIBBQAE
# aDBmAjEA5O7dFoTeoar6Xx1vCABDDvCOsODKdWuqnbGn8z7XhjIn32HIroEq5Kv9
# fU4xpW+HAjEA1s3JNQftPDx8qnYBqLqw3LLgwCo39W5Th1TkKCoRm0C7/ZXxcRIQ
# D7BK081CnsDZoYIDIzCCAx8GCSqGSIb3DQEJBjGCAxAwggMMAgEBMGowVTELMAkG
...
# BgkqhkiG9w0BCQUxDxcNMjYwODIxMDQ1NDExWjA/BgkqhkiG9w0BCQQxMgQwxm3m
# E0rG+oDn9PwVQW7YLwRBc0VjJurBAdb3ChTElSQJwyGS6tajc8AwOoNRkNzmMA0G
# CSqGSIb3DQEBAQUABIICAKMsWLzvD4+rnJH1fqIg61RkHowJPXg7GlE+5+qp+W6P
# RhDxEHvyRHzmrd/+33BQ7QARSYSXDeTV+YkMIbYLoyRjK4lH26V80tV4Ul1Z3aI0
# XZ/dbmafo7ejwNEdPzoCK7DZlzdw72vQSquOI/XT22lEP6zvnZGMM+3KX7/m/AeH
# oIVjSr1MmRCsUj61UioEbysJ2asOM3rMbw6nDjEGMeX3Xl9J9Lf3vtFU3mmpwSle
# JLkZdnTiNFBy7ro9bKXYca4fDk20QNoJ01le2j2KSeWOUths+PodGgyKF3I2C4LB
# yTO6GJSdsdxIb7RoqXDlsUSJR19VlsahuUedVpj5XFT1w1rUwpXxLNFlajdbGJoT
# tXT/iJStxUsE107pNZU1c6W8m34E1CvyoKBsBKpvDiqr/yD+Yqk2KnD6PeNy1R0z
# K23Akor3X96u9n6Ote03I4S+Dr8y5tC4tHsDqRwFoAOTaxXpmBL2wTR1Asv1hFDd
# wjBubH6XHe7Tuf+Mc+p6qnvMvWOKFtWunAys58kNjZgzMBYytqvHRjLFwd4H/fPU
# t2q54rSVGTn0E+ms1KTme53PQnVuv/Ua7EGVRnANN2YnAbjLwyzx5lI5AMIdKvLm
# XaNgD+N97G9T85DwOXizAoV0BC+KEUOGOGkY66g+l5RtNf8dENqnhUuvs/YTIQVA
