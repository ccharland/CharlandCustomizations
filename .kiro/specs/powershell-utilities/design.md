...
2. **Nested modules (`NestedModules` in the manifest)** are the mechanism for loading
   subdirectory functions. Each subdirectory ships a `.psm1` that defines (or dot-sources)
   its functions and calls `Export-ModuleMember`.
3. **Manifest `FunctionsToExport`** is the authoritative export allow-list (explicit, no
   wildcards).

`ConvertTo-CHARHashtable` follows path (2): the function is defined directly inside
`Public/PowerShell/PowerShell-Utilities.psm1` (which carries module-level comment-based
help), and that module is registered in `NestedModules` with the function name added to
`FunctionsToExport`.

> Note: unlike the older nested modules (Git, AWS) that keep one function per `.ps1` and
> have the `.psm1` only dot-source them, this module defines its function inline in the
> `.psm1`. This was a deliberate choice for a small utility module; if it grows, functions
> can be split back into sibling `.ps1` files dot-sourced by the `.psm1` without changing
> the manifest.
...
flowchart TD
    ...
    D --> E[ConvertTo-CHARHashtable defined inline]
...
Carries module-level comment-based help (`.SYNOPSIS`/`.DESCRIPTION`/`.NOTES` listing the
exported functions), defines `ConvertTo-CHARHashtable` inline, and ends with an explicit
alphabetical `Export-ModuleMember -Function` list. The file will be Authenticode-signed as
part of the build/commit flow like every other module file.
...
.DESCRIPTION
    Nested module providing general-purpose PowerShell helpers ...
    Exported functions:
