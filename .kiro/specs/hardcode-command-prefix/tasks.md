...
- [ ] 1. Core rename — standalone .ps1 files and function definitions
  - [-] 1.1 Rename `Public/Clear-AuthenticodeSignature.ps1` → `Clear-CCAuthenticodeSignature.ps1` and update `function Clear-AuthenticodeSignature` → `function Clear-CCAuthenticodeSignature` inside the file
    - Also update any `.EXAMPLE` help and cross-references within the file
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_
  - [~] 1.2 Rename `Public/Install-ProfilesFromSource.ps1` → `Install-CCProfilesFromSource.ps1` and update function definition and help
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_
  - [~] 1.3 Rename `Public/Invoke-ScriptMultiAccountRegion.ps1` → `Invoke-CCScriptMultiAccountRegion.ps1` and update function definition and help
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_
  - [~] 1.4 Rename `Public/Set-FileSignature.ps1` → `Set-CCFileSignature.ps1` and update function definition and help
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_
  - [~] 1.5 Rename `Public/Update-Powershell7.ps1` → `Update-CCPowershell7.ps1` and update function definition and help
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_
  - [~] 1.6 Rename `Public/Git/Test-CommitSignatures.ps1` → `Test-CCCommitSignatures.ps1` and update function definition and help
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_
  - [~] 1.7 Rename `Public/Git/Install-GitHooks.ps1` → `Install-CCGitHooks.ps1` and update function definition and help
    - _Requirements: 2.1, 3.1, 3.2, 5.1, 5.2_

- [ ] 2. Core rename — nested module function definitions, exports, and manifest
  - [~] 2.1 Update `AWSCustomizations.psm1` — rename all 11 function definitions and update `Export-ModuleMember` list
  ...
  - [~] 2.2 Update `CloudFormation-TemplateProcessing.psm1` — rename all 6 function definitions and update `Export-ModuleMember` list
    ...
    - _Requirements: 2.2, 4.1, 5.1, 5.2, 8.1, 8.2_
  - [~] 2.3 Update `S3Customizations.psm1` — rename `Clear-S3Bucket` → `Clear-CCS3Bucket` and update `Export-ModuleMember`
    - _Requirements: 2.2, 4.1, 5.1, 5.2_
  - [~] 2.4 Update `Audit-AWSAccount.psm1` — rename all 12 function definitions and update `Export-ModuleMember` list
    ...
    - Also update help examples and internal cross-references
    - _Requirements: 2.2, 4.1, 5.1, 5.2, 8.1, 8.2_
  - [~] 2.5 Update `GitCustomizations.psm1` — update dot-source paths to renamed `.ps1` files and update `Export-ModuleMember` list
    ...
    - _Requirements: 4.1_
  - [~] 2.6 Update `CharlandCustomizations.psd1` — replace entire `FunctionsToExport` list with CC-prefixed names and remove `DefaultCommandPrefix = 'CC'`
    - All 37 functions must appear with their new CC-prefixed names
    - The `DefaultCommandPrefix` line must be removed entirely
    - _Requirements: 1.1, 1.2, 2.3_

- [~] 3. Checkpoint — verify module loads
...
- [ ] 4. Rename and update test files
  - [~] 4.1 Rename and update AWS test files
  ...
  - [~] 4.4 Rename and update CloudFormation test files
    ...
    - _Requirements: 6.1, 6.2, 6.3, 6.4_
  - [~] 4.5 Rename and update Core test files
    ...
    - _Requirements: 6.1, 6.2, 6.3, 6.4_
  - [~] 4.6 Rename and update Git test files
    ...
    - Update all function references and dot-source paths inside each file
    - _Requirements: 6.1, 6.2, 6.3, 6.4_
  - [~] 4.7 Update `tests/New-AWSParamSplat.Tests.ps1` — check for any references to public functions and update to CC-prefixed names (private function name `New-AWSParamSplat` stays unchanged)
    - _Requirements: 6.1_
  - [~] 4.8 Update `tests/Unit/Help/HelpDiscoverability.Tests.ps1` — update any function name references to CC-prefixed names
    - _Requirements: 6.1, 6.2_

- [ ] 5. Update documentation files
  - [~] 5.1 Update `docs/QUICK-REFERENCE.md` — replace all old function names with CC-prefixed names
    - _Requirements: 7.1, 7.2_
  - [~] 5.2 Update `docs/CloudFormation-TemplateProcessing.md` — replace all old function names with CC-prefixed names
    - _Requirements: 7.1, 7.2_
  - [~] 5.3 Update `docs/AWS-Account-Audit.md` — replace all old function names with CC-prefixed names
    - _Requirements: 7.1, 7.2_
  - [~] 5.4 Update `docs/STRUCTURE.md` — update function names and file paths to reflect renames
    - _Requirements: 7.1, 7.2_
  - [~] 5.5 Update `docs/CHANGELOG.md` — add entry documenting the prefix change
    - _Requirements: 7.1_
  - [~] 5.6 Update `docs/INSTALLATION.md` — update any usage examples with CC-prefixed names
    - _Requirements: 7.1, 7.2_
  - [~] 5.7 Update `docs/TEST-PLAN.md` — update function name references
    - _Requirements: 7.1_
  - [~] 5.8 Update `docs/parameter-reference.md` — update function name references
    - _Requirements: 7.1, 7.2_
  - [~] 5.9 Update `docs/NEW-FEATURE-PARAMETERS.md` — update function name references
    - _Requirements: 7.1, 7.2_
  - [~] 5.10 Update `README.md` — update any function name references in the project root readme
    - _Requirements: 7.1, 7.2_
